        ...
        try {
            ...
            switch (function.getName()) {
                case "do_nothing":
                    ...
                    return new ChatMessage(ChatUser.TOOL, "[Saying Nothing]", null, call.getId());
                case "check_inventory":
                    ...
                    MalmoServerPlugin.inst().getLogger().info(inventory.toString());
                ...
                case "check_score":
                    ...
                    MalmoServerPlugin.inst().getLogger().info("Score Breakdown: " + scoreBreakdown);
                ...
                case "trade":
                    ...
                    return new ChatMessage(ChatUser.TOOL, "Trade request sent", null, call.getId());
                case "accept_trade":
                ...
                case "private_message":
                    ...
                    return new ChatMessage(ChatUser.TOOL, "Private Message sent to " + messagePlayer , null, call.getId());
            }
...
//        }
        ...
        do {
            ...
            MalmoServerPlugin.inst().getLogger().info("Response: " + messages.get(messages.size() - 1) + "\n\n");
            if (toolCalls != null) {
                madeToolCall = true;
    ...
    public List<String> processChatGPTMessage2(NpcParticipant npc, Thread thread) {
        ...
        for (RunStep step : openai.threads().runs(thread).steps(run).list().getData()) {
            if (step.getType() != RunStep.Type.MESSAGE_CREATION) {
                MalmoServerPlugin.inst().getLogger().info ("Assistant made step: " + step.getType());
                continue;
            }
            ...
            for (ThreadMessageContent content : message.getContent()) {
                if (content.getType() != ThreadMessageContent.Type.TEXT) {
                    MalmoServerPlugin.inst().getLogger().info ("Unhandled message content type: " + content.getType());
                    MalmoServerPlugin.inst().getLogger().info ("This will never occur since this Assistant doesn't use images.");
                }

                MalmoServerPlugin.inst().getLogger().info (((TextContent) content).getText().getValue());
...
//        List<ChatMessage> messages = npc.getGlobalMessages();
//        ChatRequest request = npc.getRequest();
//        MalmoServerPlugin.inst().getLogger().info("Sending Request for " + npc.getPlayer().getName() + "...\n\n");
//        //MalmoServerPlugin.inst().getLogger().info("Request: " + request.toString());
//        // Add the user's message to the list
//        messages.add(message);
//        MalmoServerPlugin.inst().getLogger().info("Generating Response for " + npc.getPlayer().getName() + "...");
//        npc.setGenerating(true);
