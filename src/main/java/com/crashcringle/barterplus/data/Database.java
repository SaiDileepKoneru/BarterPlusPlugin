...
//         String setup;
//         try (InputStream in = BarterPlus.inst().getResource("barterDb.sql")) {
//             // Java 9+ way
//             setup = new String(in.readAllBytes());
//         } catch (Exception e) {
//             BarterPlus.inst().getLogger().log(Level.SEVERE, "Could not read db setup file.", e);
//             throw e;
//         }
...
//             try (Connection conn = BarterPlus.inst().getConnection();
//                  PreparedStatement stmt = conn.prepareStatement(query)) {
//                 stmt.execute();
//                 stmt.close();
//             }
//         }
        BarterPlus.inst().getLogger().info("§2Database setup complete.");
    }
    
    // public PlayerStats findPlayerStatsByUUID(String uuid) throws SQLException {

    //     PreparedStatement statement = BarterPlus.inst().getConnection().prepareStatement("SELECT * FROM player_stats WHERE uuid = ?");
    ...
    public static void createTradeRequest(TradeRequest request) throws SQLException {
        ...
        // PreparedStatement statement = BarterPlus.inst().getConnection()
...
//        statement.close();
    ...
    //     PreparedStatement statement = BarterPlus.inst().getConnection().prepareStatement("UPDATE player_stats SET deaths = ?, kills = ?, blocks_broken = ?, balance = ?, last_login = ?, last_logout = ? WHERE uuid = ?");
    ...
    //     PreparedStatement statement = BarterPlus.inst().getConnection().prepareStatement("DELETE FROM player_stats WHERE uuid = ?");
